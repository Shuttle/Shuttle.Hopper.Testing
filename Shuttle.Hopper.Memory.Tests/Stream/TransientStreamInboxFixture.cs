...
public class TransientStreamInboxFixture : InboxFixture
{
    ...
    {
        await TestInboxErrorAsync(TransientStreamConfiguration.GetServiceCollection(), "transient-stream://./{0}", hasErrorTransport, isTransactional);
    }
    ...
    {
        await TestInboxThroughputAsync(TransientStreamConfiguration.GetServiceCollection(), "transient-stream://./{0}", 1000, count, 1, isTransactional);
    }
}